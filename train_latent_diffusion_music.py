...
def main(configs, config_yaml_path, exp_group_name, exp_name):
    ...
    dataset = MusicAudioDataset(configs, split="train", add_ons=dataloader_add_ons)
    ...
    val_dataset = MusicAudioDataset(configs, split="test", size_limit=300, add_ons=dataloader_add_ons)
    ...
    if resume_from_checkpoint == None:
        try:
            ...
            print("Initialise from pretrained",pretrained_ckpt)
    
            checkpoint = torch.load(pretrained_ckpt, map_location="cpu", weights_only=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config_yaml",
        type=str,
        default = "config/lmss_artificial_flow.yaml",
        required=False,
